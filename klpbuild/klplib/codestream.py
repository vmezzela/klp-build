...
class Codestream:
    ...
    def from_codestream(cls, cs, proj, patchid, kernel):
        ...
        # MICRO-6-0_Update_2
            ...
            raise ValueError("codestream name should contain either SLE or MICRO!")
    ...
    def get_src_dir(self, arch=ARCH):
        ...
        return Path(get_datadir(arch), "usr", "src", f"linux-{self.kernel}{ktype}")
    ...
    def get_boot_file(self, file, arch=ARCH):
        ...
        return Path(get_datadir(arch), "boot", fname)
    ...
    def get_cpp_dir(self, lp_name):
        """
        Get the path to the cpp directory of the current codestream.

        Args:
            lp_name (str): The name of the live patch.

        Returns:
            Path: The path to the cpp directory of the current codestream.
        """
    ...
    def get_lp_dir(self, lp_name):
        ...
        """
        return self.get_cpp_dir(lp_name)/"lp"


    def get_cpp_work_dir(self, lp_name, fname):
        """
        ...
        return self.get_cpp_dir(lp_name)/fpath
    ...
    def get_mod_path(self, arch):
        ...
        return Path(get_datadir(arch), mod_path, "modules", f"{self.kname()}")
