...
from klpbuild.klplib import file2config
from collections import defaultdict


def analyse_files(cs_list, sle_commits, gh):
    '''
    ...
    for cs in cs_list:
        for c in sle_commits[cs.name_cs()]["commits"]:
            files = gh.get_commit_files(c, inside_patch=True)
            fconfigs, _, missing = file2config.find_configs_for_files(cs, files)
...
def analyse_kmodules(cs_list, gh):
    '''
    ...
    for cs in cs_list:
        for _, dat in cs.files.items():
            ...
            supported = gh.is_module_supported(mod, cs.kernel)
