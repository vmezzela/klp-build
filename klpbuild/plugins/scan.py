...
from klpbuild.plugins.get_patches import get_patches
...
def scan(cve, conf, no_check, lp_filter):
    ...
    if not cve or no_check:
        ...
        commits = get_patches(lp_filter, cve)
    ...
    if data_missing:
        ...
        logging.info("\t%s\n", " ".join(cs_missing))
        IBS("", gh.lp_filter).download_cs_data(data_missing)
        logging.info("Done.")
    ...
    working_cs = utils.filter_codestreams(gh.lp_filter, gh.lp_skip,
